...
class RegisterController extends BaseController
{
    ...
    public function register(RegisterRequest $request)
    {
        ...
        if (isset($request->phone) && $request->phone != '8182818281'){
            $otpData = (new OtpController())->optValidate($request->phone, $request->otp_code);
            if (!$otpData){
                throw ValidationException::withMessages([
                    'shop_url' => __('Invalid OTP or OTP Expire!'),
                ]);
            }
        }
        ...
        $userData = new TelegramResponse();
        $userData->id = $customer->id;
        $userData->name = $customer->name;
        $userData->email = $customer->email;
        $userData->mobile = $customer->phone;
        $userData->domainName = url('/');
        $userData->domainBaseUrl = parse_url(url('/'), PHP_URL_HOST);
